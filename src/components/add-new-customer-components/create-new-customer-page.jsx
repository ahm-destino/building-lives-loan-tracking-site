...
export default function CreatenewCustomer({prop}){

    const navigate = useNavigate()
    const staffInfo = prop.staffInfo;
    const customersInfo = prop.customersInfo;
    ...
    // This function is to check for form filling errors and push the details to the localStorage
    function appendNewCustomer(){
        ...
        if (name === '') {
            ...
            setErrorMessage('Please Select a Loan Officer')
            return
        }
        
        // This block of code is for me to check if a phone number like that already exist in the customerInfo List and if it does, there should be an error message 
        const confirm = Array.from(customersInfo).map(element => {
            if (element.phone_number === '+234' + phoneNumber){
       ...
       //This is where the pushing of the new customers details actually occurs 
        customersInfo.unshift(newCustomer)
    ...
    return(
        <>
            ...
            <div>
                ...
                <label className="flex flex-col w-full">
                    <p className="text-base font-medium leading-normal pb-2 mt-[20px]">LOAN OFFICER</p>
                    <div className="relative flex items-center">
                       <select id='loanOfficer' onChange={handleChange} name="loan-officer" className=" border border-slate-300 w-full  rounded-lg  p-[20px] text-xl font-medium placeholder:text-lg">
                            <option value="" selected disabled>Select Loan Officer </option>
                            {
                                staffInfo.map((el)=>{
                                    return(
                                        <option name= 'loan-officer' value={el.name} > {el.name} </option>
                                     )
                                })
                            }
                       </select>
