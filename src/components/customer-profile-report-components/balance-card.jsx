export default function UserProfileReportBalanceCard({transactionHistory}){
    // This bock of code is for calculating the total money recieved by the admin
    const totalRecieved = [];

    // transactionHistory.map((el)=>{
        const filteredMoneyRecieved = transactionHistory.filter(element => {
            return element.id === 'I Recieved'            
        });
        const totalAmountRecieved = filteredMoneyRecieved.map((el)=>{
    ...
    // transactionHistory.map((el)=>{
        const filteredMoneyGiven = transactionHistory.filter(element => {
            return element.id === 'I gave Out'            
        });
        const totalAmountGiven = filteredMoneyGiven.map((el)=>{
